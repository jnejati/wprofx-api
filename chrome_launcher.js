...
Chrome(function (chrome) {
    with (chrome) {
        ...
        Network.setCacheDisabled;
         ...
         //const { result } =  Network.canEmulateNetworkConditions();
        //console.log("CanEmulateNetworkConditions " + result);
	var p1 = Network.clearBrowserCache();
  var p2 = Network.clearBrowserCookies();
  if (myArgs.length > 4) {
   // Network.emulateNetworkConditions({offline: false, latency: 5, downloadThroughput: 2000000, uploadThroughput: 2000000})
    //var p3 = Network.emulateNetworkConditions({offline: false, latency: parseInt(myArgs[4], 10), downloadThroughput: parseInt(myArgs[5], 10), uploadThroughput: parseInt(myArgs[6], 10)});
    var p3 = Promise.all([p1, p2]).then(function(){
