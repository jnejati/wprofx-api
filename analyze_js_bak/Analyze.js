...
class Analyze {
  ...
  //     // _draw.drawGraph();
  //     //common.downloadHandler(this.output, this.url);
  //     // common.downloadHandler(this.output, 'output.json');
  //
  //     // common.legendHandler();
  //     //common.metricLegendHandler();
  //     // common.depLineHandler();
  //     // common.criticalPathHandler();
            ...
            if ('url' in trace_event['args']['data']){
                 if (trace_event['args']['data']['url'].startsWith('http://web.archive.org/') &&
                  (trace_event['args']['data']['url'].split('http').length -1  ) >= 2) {
                    ...
                    // console.log(trace_event['args']['data']['url']);
  ...
  cleanArchiveURL(url){
    if (url.startsWith('http://web.archive.org/') &&
        (url.split('http').length -1  ) >= 2) {
         // console.log('FromScript url before');
         // console.log(url);
         url = 'http' + url.split('http')[2];
         if (url.endsWith('//')){
           url = url.slice(0, -1);
         }
         // console.log('FromScript url After');
         // console.log(url);
          }
       return url;
  }
      ...
      for (var i=0; i<_length; i++){
        ...
        if (net_trace['name'] == 'ResourceSendRequest'){
          ...
          if (_url.startsWith("data:")){
            ...
            // console.log('_request_id_url');
            // console.log(this.network[_request_id]['url']);
            if (_url.includes('#')){
